...
@Introspected
class DetalheChavePixResponse(chaveResponse: CarregaChavePixResponse?) { // sem val ou var. É só um parametro comum. não vira uma propriedade no Kotlin.
    ...
    val criadaEm = chaveResponse?.chave?.criadaEm?.let {
        LocalDateTime.ofInstant(Instant.ofEpochSecond(
            it.seconds, it.nanos.toLong()), ZoneOffset.UTC)
    }
    ...
    val conta = mapOf(Pair("tipo", tipoConta), // função map que recebe um objeto do tipo Pair com chave e valor. Que é transformado em JSON para resposta.
